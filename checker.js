...
// Only alerts on sealed product (boxes, tins, ETBs, etc) - singles are
// filtered out via singles-detection below (future-proof: excludes what
// looks like a single card, rather than requiring a list of known sealed
// product names - so any new sealed product type automatically passes).
// Checks all targets in PARALLEL (not one-at-a-time) - each target is a
// different website, so no single site gets hit more than once per run.
...
async function checkTarget(target, state, config, healthReport) {
  ...
  if (allItems.length > 0) {
    const relevant = allItems.filter(
      (item) =>
        (matchesKeywords(item.name, keywords) || matchesKeywords(item.url, keywords)) &&
        isSealedProduct(item.name)
    );
...
async function checkShopifyTarget(target, state, config, healthReport) {
  ...
  const relevant = products.filter(
    (p) => matchesKeywords(p.name, keywords) && isSealedProduct(p.name)
  );
