...
async function checkNewSets(state, healthReport) {
  const KEY = "_setWatch";
  ...
  if (!isFirstRun && newSets.length > 0) {
    const lines = newSets
      ...
      .join("\n");
    await sendTelegram(
      `📣 <b>New Pokémon TCG set announced</b>\n\n${lines}\n\nTo start tracking this set across your retailers, add its name to "activeSets" in config.json.`
    );
  }
...
async function main() {
  ...
  await checkNewSets(state, healthReport);
